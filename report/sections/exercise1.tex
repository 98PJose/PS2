...
\emph{conditional inversion algorithm} produces a sample from $C$ with this steps:
\[
\boxed{\;U_2\sim\mathcal{U}(0,1),\qquad U_1\;=\;C_2^{-1}(V_1\given U_2)=V_1,\quad V_1\sim\mathcal{U}(0,1).\;}
\]
...
$r_2=t_6^{-1}(U_2)\sim t_6$. Statistics are reported
...
\paragraph{Gaussian copula log-likelihood.}
...
estimator recovers the true value up to three decimals on $\ExOneNs$ observations.
...
\paragraph{Student-$t$ copula log-likelihood.}
Let $y_{it}=t_\nu^{-1}(\hat u_{it})$ and
...
$\ln L=\ExOneIDGLogLik$, a log-likelihood gap of $\ExOneIDLLGain$. 
