...
$\mathcal{U}(0,1)$ draws used as the source of randomness, while $(U_1,U_2)$
...
\]
This is exactly what every routine in \file{exercise1/copula\_sim.py} implements.
...
The Python implementation guards against $\psi\to 0$ via a \code{safe\_psi}
mask (\file{copula\_sim.py}).

\paragraph{Mixture (used in 1.d).} A copula mixture
$C=\pi C_A+(1-\pi)C_B$ is sampled by drawing a Bernoulli($\pi$) indicator
and selecting $C_A$ or $C_B$ component-wise.

% ---------------------------------------------------------------
\subsection{1.a -- Scatter plots and dependence measures}
% ---------------------------------------------------------------

For each copula we draw $N=10^5$ pairs and compute the three classical
dependence measures: Pearson's $\rho$, Spearman's $\rho_S$ and Kendall's $\tau$.
Pearson is sensitive to the marginals, while Spearman and Kendall are
\emph{copula-only} invariants.
Numerical results (Table~\ref{tab:1a}) and scatter plots
(Figure~\ref{fig:1a}) show:
\begin{itemize}\setlength\itemsep{0pt}
\item Clayton concentrates mass in the south-west corner (lower-tail dependence).
\item Survival Clayton concentrates mass in the north-east corner (upper-tail dependence).
\item Gaussian and Student-$t$ at $\rho=-0.7$ produce the elliptical anti-diagonal pattern;
...
\paragraph{Sanity check on Kendall's $\tau$.} For Clayton,
...
This is a strong validation of the simulator.
...
$\alpha$-quantile $\widehat{\mathrm{VaR}}_\alpha=\hat F_{r_p}^{-1}(\alpha)$, computed
in \code{exercise1.portfolio.compute\_stats} with \code{numpy.quantile}.
...
The shift from $T$ to $T+1$ avoids exact 0/1 values that would blow up
$\Phi^{-1}(\hat u)$.
...
(\code{exercise1.estimation.estimate\_gaussian\_copula}).
...
same data gives $\hat\rho=0.9057$ and $\ln L=85{,}791$, more than $11{,}000$
log-likelihood points behind. This is a textbook example of how IFM correctly
