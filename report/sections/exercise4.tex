...
\emph{Option-based portfolios} using a Python port of
\file{ccK\_simuRet2.m}.
...
(scenarios 4.1--4.4 below). $N=\ExFourN$ paths and a strike grid
...
\caption{Strategy statistics at $K=100$, $N=\ExFourN$ Monte-Carlo paths.}\label{tab:4-atm}
...
\begin{itemize}\setlength\itemsep{0pt}
\item \emph{Gaussian, high $\rho=0.9$}: when the held asset 2 is almost a
      copy of asset 1 the option hedge is almost perfect, leaving roughly the
      same risk profile as \code{cc}.
\item \emph{Gaussian, low $\rho=0.3$}: \code{cc2} std jumps
...
\item \emph{Student $t$ at the same $\rho=0.3$}: similar std to the Gaussian
      case, but kurtosis rises thanks to  $\nu=5$. The protective put inherits a heavier upper tail.
\item \emph{Clayton $\theta=10$}: lower-tail dependence collapses
      \code{cc2} std back to the single-asset level -- when $S_{1T}$ is small,
      $S_{2T}$ is also small with high probability. Skew flips sign because
      the call's truncation no longer captures the asset-2 upside
      symmetrically.
\end{itemize}
