...
We work with the daily log-returns of three large-cap crypto assets
...
The three series exhibit textbook crypto stylised facts: near-zero mean,
markedly negative skew (especially BTC and ETH), and a pronounced excess
kurtosis (Pearson kurtosis $\gg 3$). High pair-wise correlations justify a
\emph{joint} dependence model.
...
$\hat\Omega_{\textsc{eth-ada}}=0.7418$. The persistence is essentially 1 --- a
typical IGARCH-style finding driven by the strong persistence of the EDF
ranks themselves. See Figure~\ref{fig:22a}.
...
Figure~\ref{fig:23} overlays the three pair-wise correlation paths for the
...
The three series share the same qualitative pattern --- correlations rise
during the COVID-19 shock (March 2020), the May 2021 sell-off, the FTX
collapse (Nov 2022), and the late-2023 risk-on rally. Quantitatively, EDF
margins produce the smoothest path (the rank transformation truncates extreme
co-movements), while GARCH/GJR margins capture sharper short-run drops in
dependence. The $\hat a+\hat b$ estimate falls from $0.9999$ (EDF) to $\approx
0.96$ once the marginal variance is filtered, which is the standard outcome
when margins absorb most of the persistence.
