...
% Keep TOC compact when parskip is enabled in body text
...
% Allow compilation either from this directory or from the project root.
\newcommand{\reportdir}{}
\IfFileExists{generated/ex1.tex}{}{\renewcommand{\reportdir}{report/}}
...
% ---------- Auto-generated numeric macros (populated by Python mains) ----------
\input{generated/ex1.tex}
\input{generated/ex2.tex}
\input{generated/ex3.tex}
\input{generated/ex4.tex}
\input{generated/ex5.tex}
\input{generated/ex6.tex}
...
$U_1$ produces a sample with dependence coming from the desired copula. We adopt the notation
...
\file{exercise6/} & Lo (2007) stop-loss policy, eq.~(26) verification\\
...
\file{config.json} and writes its graphs to \file{figures/} and other results to \file{report/generated/}.
...
\item Exercise~\ref{sec:ex1} shows how a single dependence parameter can produce
      ...
      dependence parameter up to three decimals.
\item Exercise~\ref{sec:ex2} confirms that crypto returns exhibit the standard
      ...
      similar dynamic correlation paths; switching from a
...
\item Exercise~\ref{sec:ex6} verifies Lo's (2007) closed-form
      equation~(26) for the expected return of a stop-loss-modified
      AR(1) portfolio. The Monte-Carlo experiment matches Lo's
      Table~1 to one decimal place across the entire
      $(\rho,\zeta)$ grid, with a maximum deviation of $\ExSixMaxDiff\%$
      (annualised). The active component is positive only under
      momentum ($\rho>0$): with $\rho=+50\%$ the stop-loss adds up to
      $13.8\%$ to the annualised return, while at $\rho=-25\%$ it
      destroys roughly $7\%$ regardless of the threshold.
\end{enumerate}
...
\item Lo, A. W. (2007). \emph{Where Do Alphas Come From? A Measure of the
      Value of Active Investment Management}. Journal of Investment
      Management, 6(2).
